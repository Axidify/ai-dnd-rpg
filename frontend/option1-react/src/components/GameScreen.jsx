              ...
              <div className="p-4">
                {quests && quests.length > 0 ? (
                    ...
                    {quests.map((quest, index) => (
                      <div
                        ...
                        className={`p-4 rounded-lg border ${
                          quest.status === 'COMPLETE' || quest.status === 'completed'
                            ? 'bg-green-900/20 border-green-700' 
                            : quest.status === 'FAILED' || quest.status === 'failed'
                            ? 'bg-red-900/20 border-red-700'
                            : 'bg-rpg-dark/50 border-gray-700'
                        }`}
                      >
                        <div className="flex items-start gap-3">
                          <div className="mt-1">
                            {quest.status === 'COMPLETE' || quest.status === 'completed' ? (
                              ...
                              <Circle size={20} className="text-rpg-gold" />
                            )}
                          </div>
                          <div className="flex-1">
                            <h3 className="font-semibold text-rpg-gold">{quest.name}</h3>
                            <p className="text-sm text-rpg-text-secondary mt-1">{quest.description}</p>
                            
                            {/* Objectives */}
                            {quest.objectives && quest.objectives.length > 0 && (
                              <div className="mt-3 space-y-1">
                                <div className="text-xs text-rpg-text-secondary font-semibold">Objectives:</div>
                                {quest.objectives.map((obj, i) => (
                                  <div key={i} className="flex items-center gap-2 text-sm">
                                    ...
                                    <span className={obj.completed ? 'line-through text-gray-500' : ''}>
                                      {obj.description}
                                      {(obj.current_count !== undefined && obj.required_count !== undefined) ? (
                                        <span className="text-rpg-primary ml-1">
                                          ({obj.current_count}/{obj.required_count})
                                        </span>
                                      ) : (obj.current !== undefined && obj.required !== undefined) && (
                                        <span className="text-rpg-primary ml-1">
                            ...
                            {quest.rewards && (
                              <div className="mt-3 flex gap-3 text-xs">
                                {quest.rewards.gold && (
                                  <span className="text-rpg-gold">💰 {quest.rewards.gold} Gold</span>
                                )}
                                {quest.rewards.xp && (
                                  <span className="text-yellow-400">⭐ {quest.rewards.xp} XP</span>
                                )}
                                {quest.rewards.items && quest.rewards.items.length > 0 && (
                                  <span className="text-purple-400">📦 {quest.rewards.items.join(', ')}</span>
                                )}
                              </div>
                            )}
                          </div>
                  ...
                  <div className="text-center py-8 text-rpg-text-secondary">
                    ...
                    <p className="text-sm mt-2">Talk to NPCs to find quests!</p>
                  </div>
                )}
              </div>
            </motion.div>
