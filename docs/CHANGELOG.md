...
### Added
- **AI DM Mechanics Testing - Round 11.3 FINAL** - Achieved 95.5% pass rate!
  - Fixed all test navigation to use proper `/api/travel` endpoint
  - Updated all tests to use `goblin_cave` structured scenario
  - Corrected exit names to match scenario definitions ("bar", "forge", "outside", etc.)
  - All 21 main tests now PASSING
  - Only 1 warning (gold verification timing - not DM issue)

- **AI DM Mechanics Testing - Round 11** - Comprehensive testing of DM tag generation!
  - Created `tests/test_dm_mechanics_round10.py` with 45 comprehensive tests
  - Created `tests/test_dm_focused_round11.py` with 22 focused scenario tests
  - Validated all 12 skill types generate correct [ROLL:] tags
  - Validated [COMBAT:], [BUY:], [PAY:], [RECRUIT:] tag generation
  - Verified prompt injection resistance (AI stays in character)
  - Verified no auto-travel, no invented NPCs, no reroll spam
  - **Final Pass rate: 95.5%** âœ… 
  - Created `docs/DM_TESTING_COMPLETE_SUMMARY.md` comprehensive report
  - Created `docs/DM_TESTING_ROUND11.md` detailed findings

- **AI DM Prompt Improvements** - Major enhancements to dm_engine.py!
  - Added Intimidation keyword triggers ("or else", "NOW!", "threaten", etc.)
  - Added TAG OUTPUT CHECKPOINT for self-verification before responses
  - Added Gold Trust Rule - DM trusts player-stated amounts
  - Added DURING ACTIVE COMBAT rules - prevents DM from narrating attack outcomes
  - Added QUEST REWARD CHECK and LOOT/DISCOVERY CHECK
  - Added SKILL CHECK OUTCOME NARRATION with meaningful failure consequences
  - Added MULTI-ENEMY COUNT ACCURACY - matches enemies to location data
  - Added ATMOSPHERE & CONTEXT AWARENESS section (time, weather, party, NPC knowledge)
  - Improved [PAY:] and [RECRUIT:] tag generation from 0% to ~100%
  - Improved Intimidation recognition from ~50% to ~90%

- **Test Speed Optimization** - 50% faster test execution!
  - Reduced all `time.sleep()` delays (1.5sâ†’0.7s, 2sâ†’1s, 1sâ†’0.5s)
...
- **Party Combat Integration (Priority 7 Step 6)** - Complete party member combat system!
  ...
  - `format_party_member_attack()` with class-specific emoji formatting (ðŸ›¡ï¸ Fighter, ðŸ¹ Ranger, etc.)
...
- **Reputation Command System** - View NPC disposition standings!
  ...
  - Disposition tier labels with color-coded emojis (ðŸ”´ Hostile, ðŸŸ  Unfriendly, ðŸŸ¡ Neutral, ðŸŸ¢ Friendly, ðŸ’š Trusted)
...
- **Phase 3.6 Item Utility Quick Wins** - Unused items now have mechanical purpose!
  ...
  - **"Thin the Herd" Bounty Quest** - New side quest for goblin_ear collection!
    * Collect 5 goblin ears â†’ 25g + 50 XP
...
- **Phase 3.6.7 Torch Darkness Mechanics** - Dark locations require light! âœ… NEW
...
- **Phase 3.6.7b Darkness Combat Integration** - Combat now applies darkness disadvantage! âœ… NEW
...
- **Phase 3.6.8 Rope Utility** - Rope enables cage escape! âœ… NEW
...
**ðŸŽ‰ Phase 3.6 Item Utility System: COMPLETE (8/8 features)**
...
- **Barkeep NPC Location Dialogue** - Barkeep no longer implies all recruitable NPCs are in tavern!
  ...
  - AI now knows: Marcus â†’ TAVERN, Elira â†’ FOREST, Shade â†’ CAVE
...
- **NPC Location Filtering** - NPCs now appear only at their assigned locations!
  ...
  - Added NPCs to their correct `Location.npcs` lists in scenario:
    * Marcus â†’ tavern_main
    * Elira â†’ forest_clearing  
    * Shade â†’ goblin_camp_shadows
...
- **Exceptional Roleplay XP Guidelines** - Clear criteria for AI discretionary XP!
  ...
  - Explicit âœ… AWARD and âŒ DON'T AWARD examples
...
- **DM System Prompt XP Guidelines** - More explicit XP rules for AI DM
  ...
  - Added real gameplay examples with âœ…/âŒ markers
...
- **Emoji Encoding (Mojibake Fix)** - Emojis now display correctly!
  - Fixed double-encoded UTF-8 emojis appearing as garbled text (e.g., `Ã°Å¸"` instead of `ðŸ“`)
...
- **Quest Journal Tracking** - Quests now properly activate and track!
  ...
  - Fixed duplicate Flask endpoint causing startup error (`combat_status` â†’ `combat_status_v2`)
...
- **Shop Display Format** - Enhanced item listings with stats
  - Example: `Longsword (x2) [DMG: 1d8].................17g âœ“`
  - Example: `Studded Leather (x1) [AC: +2].............51g âœ—`

- **Inventory Display Format** - Stats shown inline with items
  - Example: `â€¢ Longsword [DMG: 1d8]`
  - Example: `â€¢ Studded Leather [AC: +2]`
...
- **Travel Menu System** - Streamlined navigation with two-phase travel!
  ...
  - Danger indicators: âš ï¸ threatening, â˜ ï¸ deadly, â“ uneasy, âœ“ visited
...
- **Combat Balance Overhaul** - Enemies retuned for better survivability!
  - Goblin: HP 7â†’5, AC 15â†’12, Attack +4â†’+3, Damage 1d6+2â†’1d6+1
  - Wolf: HP 11â†’8, AC 13â†’11
  - Giant Spider: HP 26â†’18, AC 14â†’13
...
### Changed
- **Development Plan Phase 5.2** - JSON Data Refactor â†’ Scenario Folder System
...
- **Enhanced Shop Interaction System** - More natural shopping experience!
  ...
  - Shows affordable quantity when gold is insufficient: "ðŸ’¡ You can afford 4"
...
- **Flexible Travel System** - More forgiving movement commands!
  ...
  - Smart shop redirect: "go to blacksmith" â†’ hints to use `shop` command
...
- **XP Progress Display** - See your progress to the next level!
  - After receiving XP, shows: `ðŸ“ˆ Level 2: 50/100 XP (50 needed)`
  - At max level shows: `ðŸ“ˆ Max Level reached! (1250 total XP)`
...
- Disposition foundations added!
  ...
  - Enhanced NPC methods:
    ...
    - `get_disposition_label()` - Returns formatted label with emoji (e.g., "ðŸŸ¢ Friendly (+35)")
...
- **Verified Security Architecture**: Tag parsers only process DM responses, not player input
  - Player input â†’ AI model â†’ DM response â†’ Parsers â†’ Game state
...
- **Shop Stock Tracking System** - Merchants now have limited inventory!
  ...
  - Shop display now shows quantities: "Healing Potion (x6)" or "(âˆž)" for unlimited
...
- **Cardinal Direction Aliases (Phase 3.2.1 Priority 6)** - Classic text adventure navigation
  ...
  - `resolve_direction_alias()` helper: expands 'n' â†’ 'north', etc.
...
- **Developer Guide: Scenario System Documentation**
  ...
  - Hierarchy diagram: ScenarioManager â†’ Scenario â†’ Scene â†’ Location
...
- **Location Narration System (Phase 3.2.1 Priority 4)** - AI-generated immersive location descriptions
  ...
  - `display_location_narration()`: Displays ðŸ“ narration with consistent format
...
## Historical Changes (2024)

For changelog entries from December 2024 (versions 0.1.0 - 0.9.1), see:
- [archive/CHANGELOG_2024.md](archive/CHANGELOG_2024.md)
